...
public class Service : IService
{
    ...
    public delegate void MyEventHandlerForPurchase(Client client, Tariff tariff);

    public delegate void MyEventHandlerForAdd<T>(T value);

    public event MyEventHandlerForAdd<Tariff> TariffAdded;  
    public event MyEventHandlerForAdd<Client> ClientAdded;
    public event MyEventHandlerForPurchase Purchased;

    public void OnTariffAdded(Tariff tariff)
    {
        TariffAdded?.Invoke(tariff);
    }
    ...
    public void OnPurchased(Client client, Tariff tariff)
    {
        Purchased?.Invoke(client, tariff);
    }
    ...
    public void AddTariff(string name, double price)
    {
        ...
        _listTariffs.Add(tariff);
        OnTariffAdded(tariff);
    }
    ...
    private void AddClientTariff(string tariff)
    {
        ...
        _listClients.Current()._tariffList.Add(_listTariffs.Current());
        OnPurchased(_listClients.Current(), _listTariffs.Current());
    }
    ...
    public double GetSumClient(string name)
    {
        ...
        if (!flag)
            return 0;
        var currentTariffs = _listClients.Current()._tariffList;
    ...
    public double GetTotalSum()
    {
        ...
        {
            var listTariff = _listClients[i]._tariffList;
    ...
    public int CountOrders(string name)
    {
        ...
        for (int i = 0; i < _listClients.Count; i++)
        {
            var listClientsTariff = _listClients[i]._tariffList;
